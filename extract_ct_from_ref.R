...
extract_reference  <- function(body_organ, cell_hierarchy_cols) {
  ...
  names(cell_types) <- sapply(1:length(cell_hierarchy_cols), function (x) return(stringr::str_interp("AS/${x}")))
...
for (reference in fromJSON(file = 'organ_data.json')$references) {
